...
class TestView (TestCase):
    ...
    def test_detect_delivery_problem(self):
        ...
        product = Product(name="Create a Test", price=1, available_quantity=10, euro_shipping=False)
        product.save()
        session = self.client.session
        session['bag'] = {product.id: 1 }
        session.save()
        #user is not logged in(has not specified a delivery address in  Ireland)
        response = self.client.get('/checkout/')
        ...
        #user is logged in with delivery addess in Ireland but selects a delivery to a EU country
        session = self.client.session
